...
class GeneratorState(nn.Module):
    def __init__(self, latent_size=64, state_size=128, motion_size=16, num_feature=128):
        '''
        Input:
    ...
    def forward(self, z, state, motion=None):
        # print(z.shape,state.shape,motion.shape)
...
def G_forward_perview(GStateNet, GRainNet, truncate_Z, initial_state, motion_type):

    B = truncate_Z.shape[0]
    # print(truncate_Z.shape)  #torch.Size([1, 50, 64])
    # print('truncate_Z shape', truncate_Z.shape)
    input_Z = truncate_Z.view([B,-1])
    input_S = initial_state
    input_M = motion_type.view([B,-1])
    state_next = GStateNet(input_Z, input_S,input_M)
    rain_gen = GRainNet(state_next)             
    return rain_gen
