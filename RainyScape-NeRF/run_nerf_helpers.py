...
def get_rays(H, W, K, c2w):
    ...
    i = i.t()
    j = j.t()
    dirs = torch.stack([(i - K[0][2] + HALF_PIX)/K[0][0], -(j - K[1][2] + HALF_PIX)/K[1][1], -torch.ones_like(i)], -1)
...
######
# def get_rays_rgb(H, W, K, c2w, image):
#     i, j = torch.meshgrid(torch.linspace(0, W-1, W), torch.linspace(0, H-1, H))  # pytorch's meshgrid has indexing='ij'
#     i = i.t()
#     j = j.t()
...
def get_rays_np(H, W, K, c2w):
    ...
    # print(rays_o.shape,rays_d.shape)  # [512,512,3]
    return rays_o, rays_d
